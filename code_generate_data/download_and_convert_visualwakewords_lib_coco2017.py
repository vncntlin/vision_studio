...
def _create_tf_example(image, annotations, image_dir):
  ...
  return example
#Dues
def dues_create_visual_wakeword_annotations(annotations_file,
                                       visualwakewords_annotations_file,
                                       small_object_area_threshold,
                                       foreground_class_of_interest,
                                       image_dir,
  ...
  """
  # default object of interest is person
  new_data_dir_noobj = os.path.join(new_data_dir, 'no_object')
  new_data_dir_obj = os.path.join(new_data_dir, 'object')
  foreground_class_of_interest_id = 1
  with tf.gfile.GFile(annotations_file, 'r') as fid:
    groundtruth_data = json.load(fid)
    images = groundtruth_data['images']
    ...
    for idx, image in enumerate(images):
      if idx % 100 == 0:
        tf.logging.info('On image %d of %d', idx, len(images))
      annotations = annotations_index[image['id']]
      label_filtered = dues_filter_annotations(
          annotations, image, small_object_area_threshold,
          foreground_class_of_interest_id)
      len(str(image['id']))
      image_name = 'COCO_' + name_prefix + '2014_' + str(image['id']).zfill(12) + '.jpg'
      name_origin = os.path.join(image_dir, image_name)
      image_origin = cv2.imread(name_origin)
      image_gray = cv2.cvtColor(image_origin,cv2.COLOR_BGR2GRAY)
      image_gray = cv2.resize(image_gray, (96,96))
      if(label_filtered==0):
        name_new = os.path.join(new_data_dir_noobj, image_name)
        cv2.imwrite(name_new, image_gray)
        #shutil.copyfile(name_origin, name_new)
      elif(label_filtered==1):
        name_new = os.path.join(new_data_dir_obj, image_name)
        cv2.imwrite(name_new, image_gray)
        #shutil.copyfile(name_origin, name_new)
      else:
       raise Exception("Dues label fail")
      

def dues_filter_annotations(annotations, image, small_object_area_threshold,
                        foreground_class_of_interest_id):
  """Filters COCO annotations to visual wakewords annotations.
...
def dues_2class_create_visual_wakeword_annotations(annotations_file,
                                       visualwakewords_annotations_file,
                                       small_object_area_threshold,
                                       foreground_class_of_interest_1,
                                       foreground_class_of_interest_2,
                                       image_dir,
                                       new_data_dir,
                                       name_prefix):
  """Generate visual wakewords annotations file.
  ...
  new_data_dir_noobj = os.path.join(new_data_dir, 'no_object')
  new_data_dir_obj = os.path.join(new_data_dir, 'object')
  foreground_class_of_interest_id_1 = 1
  foreground_class_of_interest_id_2 = 2
  with tf.gfile.GFile(annotations_file, 'r') as fid:
    groundtruth_data = json.load(fid)
    images = groundtruth_data['images']
    # Create category index
    category_index = {}
    for category in groundtruth_data['categories']:
      if category['name'] == foreground_class_of_interest_1:
        foreground_class_of_interest_id_1 = category['id']
        category_index[category['id']] = category
      if category['name'] == foreground_class_of_interest_2:
        foreground_class_of_interest_id_2 = category['id']
        category_index[category['id']] = category
    # Create annotations index, a map of image_id to it's annotations
    tf.logging.info('Building annotations index...')
    annotations_index = collections.defaultdict(
    ...
    for idx, image in enumerate(images):
      if idx % 100 == 0:
        tf.logging.info('On image %d of %d', idx, len(images))
      annotations = annotations_index[image['id']]
      label_filtered = dues_2class_filter_annotations(
          annotations, image, small_object_area_threshold,
          foreground_class_of_interest_id_1, foreground_class_of_interest_id_2)
      len(str(image['id']))
      image_name = str(image['id']).zfill(12) + '.jpg'
      name_origin = os.path.join(image_dir, image_name)
      image_origin = cv2.imread(name_origin)
      image_gray = cv2.cvtColor(image_origin,cv2.COLOR_BGR2GRAY)
      image_gray = cv2.resize(image_gray, (96,96))
...
def dues_2class_filter_annotations(annotations, image, small_object_area_threshold,
                        foreground_class_of_interest_id_1, foreground_class_of_interest_id_2):
  """Filters COCO annotations to visual wakewords annotations.
  ...
  for annotation in annotations['objects']:
    ...
    category_id = int(annotation['category_id'])
    # Filter valid bounding boxes
    if ((category_id == foreground_class_of_interest_id_1) or \
        (category_id == foreground_class_of_interest_id_2)) and \
        normalized_object_area > small_object_area_threshold:
      objects.append({
          u'area': annotation['area'],
          u'bbox': annotation['bbox'],
